...
class BaseGameViewController: UIViewController {
    ...
    var LawrenceOfArabia = ItemsArray.fetchLawrenceOfArabia()
    var WelcomeToTheJungle = ItemsArray.fetchWelcomeToTheJungle()
    var SiberianTiger = ItemsArray.fetchSiberianTiger()
    ...
    func xtraImages(){
        ...
        xtraTopFrame.frame = CGRect(x: 347, y: -1, width: 60, height: 20)
    ...
    func getTheTeritorrys(map: String) {
        ...
        case "Strangerson Island":
            ...
            teritorryNumber = number
            if let name = UserDefaults.standard.string(forKey: "uName"){
                StrangersonIsland[number].title = "Capitol " + StrangersonIsland[number].title
        ...
        case "Siberian Tiger":
            ...
            teritorryNumber = number
            if let name = UserDefaults.standard.string(forKey: "uName"){
                SiberianTiger[number].title = "Capitol " + SiberianTiger[number].title
                SiberianTiger[number].possessor = name
                SiberianTiger[number + 1].possessor = name
                SiberianTiger[number + 2].possessor = name
        ...
        case "Welcome to the Jungle":
            ...
            teritorryNumber = number
            if let name = UserDefaults.standard.string(forKey: "uName"){
                WelcomeToTheJungle[number].title = "Capitol " + WelcomeToTheJungle[number].title
                WelcomeToTheJungle[number].possessor = name
                WelcomeToTheJungle[number + 1].possessor = name
                WelcomeToTheJungle[number + 2].possessor = name
        ...
        case "Lawrence of Arabia":
            ...
            teritorryNumber = number
            if let name = UserDefaults.standard.string(forKey: "uName"){
                LawrenceOfArabia[number].title = "Capitol " + LawrenceOfArabia[number].title
                LawrenceOfArabia[number].possessor = name
                LawrenceOfArabia[number + 1].possessor = name
                LawrenceOfArabia[number + 2].possessor = name
...
extension BaseGameViewController: BuldingTableViewProtocol{
    ...
    func onClickBiuld(index: Int, image: String, price: Int, title: String) {
        ...
        for i in playerTeritorrys{
            if i.title == StrangersonIsland[teritorryNumber].title{
        ...
        if tit == "Capitol" {
            if let first = StrangersonIsland[teritorryNumber].title.components(separatedBy: " ").first {
                if first == "Capitol"{
                    //check if you have enuf money
                    if lvBit >= prices  {
                        if StrangersonIsland[teritorryNumber].numberOfBuldings > 0{
                            //reduct the price of the biulding from the actual bank
                            lvBit -= prices
                            //reduct the number of biulding by 1
                            StrangersonIsland[teritorryNumber].numberOfBuldings -= 1
                            //refresh the bank
                            bitTextField.text = "\(lvBit)"
                            waterTextField.text = "\(water)"
                            titaniumTextField.text = "\(titanium) "
                            uraniumTextField.text = "\(uranium) "
                            //check if ther is biuldings in the teritorry
                            if StrangersonIsland[teritorryNumber].biuldings == ["None"]{
                                StrangersonIsland[teritorryNumber].biuldings = [tit]
                                buildingsArray.remove(at: indexx)
                               
                                playerTeritorrys[counter].biuldings = [tit]
                                
                            }else{
                                StrangersonIsland[teritorryNumber].biuldings = StrangersonIsland[teritorryNumber].biuldings + [tit]
                                playerTeritorrys[counter].biuldings = playerTeritorrys[counter].biuldings + [tit]
                                print(playerTeritorrys)
                                
                                
                            }
                        }else{
                            let alert = UIAlertController(title: "No room to biuld", message: " in this teritorry there is no more spots to biuld, try somwhere else." , preferredStyle: UIAlertController.Style.alert)
                    ...
                    let alert = UIAlertController(title: "you cant biuld a Capitol here", message: " Capitol can be biuld only in the capitol city." , preferredStyle: UIAlertController.Style.alert)
                    // add an action (button)
                    alert.addAction(UIAlertAction(title: "Got It", style: UIAlertAction.Style.default, handler: nil))
                    //show the alert
                    self.present(alert, animated: true, completion: nil)
        ...
        let name = Notification.Name(rawValue: notificationBild)
